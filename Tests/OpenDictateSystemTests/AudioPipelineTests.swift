...
struct AudioPipelineTests {
    ...
    func fixture(seconds: Double, start: Double, end: Double, amplitude: Float, aac: Bool = false) throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "audio-test-\(UUID()).\(aac ? "m4a" : "wav")")
        let format = try #require(AVAudioFormat(standardFormatWithSampleRate: 24_000, channels: 1))
        let buffer = try #require(
            AVAudioPCMBuffer(pcmFormat: format, frameCapacity: AVAudioFrameCount(seconds * 24_000)))
    ...
    func benchmark() async throws {
        ...
        for index in 0..<7 {
            ...
            let bytes =
                (try FileManager.default.attributesOfItem(atPath: export.path)[.size] as? NSNumber)?.intValue ?? 0
            rows.append(
                "export,\(index),\(Double(d.components.seconds)*1000+Double(d.components.attoseconds)/1e15),\(bytes)")
